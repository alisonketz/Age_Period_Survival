...
\hypersetup{
            pdftitle={Appendix S5 White-tailed deer Age-Period Survival Analysis R Code},
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{d_fit <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"../wtd_data.csv"}\NormalTok{)}
\NormalTok{n_fit <-}\StringTok{ }\KeywordTok{dim}\NormalTok{(d_fit)[}\DecValTok{1}\NormalTok{]}

\CommentTok{#vector to calibrate indexes}
\NormalTok{age2date <-}\StringTok{ }\NormalTok{d_fit}\OperatorTok{$}\NormalTok{left.time }\OperatorTok{-}\StringTok{ }\NormalTok{d_fit}\OperatorTok{$}\NormalTok{ageCapture}

\NormalTok{### Number of Age effects and Number of Period effects}
\NormalTok{nT_age <-}\StringTok{ }\KeywordTok{max}\NormalTok{(d_fit}\OperatorTok{$}\NormalTok{ageRight) }\OperatorTok{-}\StringTok{ }\DecValTok{1}
\NormalTok{nT_period <-}\StringTok{ }\KeywordTok{max}\NormalTok{(d_fit}\OperatorTok{$}\NormalTok{right.time) }\OperatorTok{-}\StringTok{ }\DecValTok{1}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{knots_age_cgam <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\KeywordTok{round}\NormalTok{(}\KeywordTok{quantile}\NormalTok{(d_fit}\OperatorTok{$}\NormalTok{ageRight }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{,}
...
\NormalTok{knots_age_cgam <-}\StringTok{ }\KeywordTok{unique}\NormalTok{(knots_age_cgam)}
\NormalTok{delta_i <-}\StringTok{ }\KeywordTok{convex}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{nT_age, knots_age_cgam, }\DataTypeTok{pred.new =} \OtherTok{FALSE}\NormalTok{)}
...
\NormalTok{knots_age_spline <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}
                   \KeywordTok{round}\NormalTok{(}\KeywordTok{quantile}\NormalTok{(d_fit}\OperatorTok{$}\NormalTok{ageRight }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{,}
...
\NormalTok{##A constraint matrix so time.effects = 0}
...
\NormalTok{Z <-}\StringTok{ }\KeywordTok{qr.Q}\NormalTok{(qrc, }\DataTypeTok{complete =} \OtherTok{TRUE}\NormalTok{)[, (}\KeywordTok{nrow}\NormalTok{(constr_sumzero) }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{)}\OperatorTok{:}\KeywordTok{ncol}\NormalTok{(constr_sumzero)]}
...
\NormalTok{nimData <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{censor =}\NormalTok{ d_fit}\OperatorTok{$}\NormalTok{censor,}
                ...
                \DataTypeTok{left_age =}\NormalTok{ d_fit}\OperatorTok{$}\NormalTok{ageCapture,}
                \DataTypeTok{right_age =}\NormalTok{ d_fit}\OperatorTok{$}\NormalTok{ageRight,}
...
\NormalTok{initsFun <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{()}\KeywordTok{list}\NormalTok{(}
                          ...
                          \DataTypeTok{beta0_temp =} \KeywordTok{rnorm}\NormalTok{(}\DecValTok{1}\NormalTok{, }\OperatorTok{-}\DecValTok{5}\NormalTok{, .}\DecValTok{0001}\NormalTok{),}
                          \DataTypeTok{mix =} \DecValTok{1}\NormalTok{,}
                          \DataTypeTok{mix2 =} \DecValTok{1}\NormalTok{,}
                          \DataTypeTok{sda_period =} \KeywordTok{runif}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{3}\NormalTok{),}
