...
ggsave("timeeffect.pdf", period_effect_plot)
...
sink("results_wtd_CAR.txt")
print(fit_sum[c(grep("mu_age",rownames(fit_sum)),grep("tau",rownames(fit_sum))),])
print(gelman.diag(out[,c(grep("mu_age",rownames(fit_sum)),grep("tau",rownames(fit_sum)))]))
print(effectiveSize(out[,c(grep("mu_age",rownames(fit_sum)),grep("tau",rownames(fit_sum)))]))
cat("Runtime: ",runtime,"\n")
cat("Reps: ",reps,"\n")
print(mcmcout$WAIC)
sink()
...
ess <- effectiveSize(out[,c(grep("mu_age",rownames(fit_sum)),
                            grep("tau",rownames(fit_sum)))])
save(ess,file="ess.Rdata")
gd <- gelman.diag(out[,c(grep("mu_age",rownames(fit_sum)),
                         grep("tau",rownames(fit_sum)))])
