...
pdf("traceplots_K_K.pdf")
traceplot(out[, "beta0"], ylab = "beta0")
traceplot(out[, "sdk_age"], ylab = "sdk_age")
traceplot(out[, "sda_age"], ylab = "sda_age")
traceplot(out[, "ratioinf_age"], ylab = "ratioinf_age")
...
te_indx <- grep("period_effect", rownames(fit_sum))[1:nT_period]
...
print(gelman.diag(out[,c(grep("beta", rownames(fit_sum)),
                         grep("tau", rownames(fit_sum)),
                         grep("sd", rownames(fit_sum)))],multivariate=FALSE))
...
gd <- gelman.diag(out[, c(grep("beta", rownames(fit_sum)),
                        grep("sd", rownames(fit_sum)),
                        grep("tau", rownames(fit_sum)))],multivariate=FALSE)
