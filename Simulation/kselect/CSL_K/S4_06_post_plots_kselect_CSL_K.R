...
traceplot(out[, "period_effect[103]"], ylab = "period_effect[103]")
...
age_effect_plot_cgam <- ggplot(data = out_age_effect, aes(x = intervals)) +
...
ggsave("age_effect_cgam.pdf", age_effect_plot_cgam)

##################################
### 
### Log hazard - Age effect spline
###
##################################
...
age_effect_plot_spline <- ggplot(data = out_age_effect, aes(x = intervals)) +
  geom_line(aes(x = intervals, y = age_effect_mean), size = 1) +
  geom_ribbon(aes(ymin = age_effect_lower, ymax = age_effect_upper),
              alpha = .2,
              linetype = 0)+
  ggtitle("Age Effect Posterior") +
...
ggsave("age_effect_spline.pdf", age_effect_plot_spline)


##################################
### 
### Log hazard - Age effect 
### cgam and spline combined
###
##################################

te_indx <- grep("age_effect", rownames(fit_sum))
age_effect_mean <- fit_sum[te_indx, 1][1:nT_age]
age_effect_lower <- fit_sum[te_indx, 4][1:nT_age]
age_effect_upper <- fit_sum[te_indx, 5][1:nT_age]
intervals <- 1:nT_age
out_age_effect <- data.frame(intervals,
                             age_effect_mean,
                             age_effect_lower,
...
ggsave("age_effect_combo.pdf",age_effect_plot)
...
###############################################
###
### Save results for assessment
###
##############################################

sink("results_kselect_CSL_K.txt")
print(fit_sum[c(grep("beta", rownames(fit_sum)),
          ...
          grep("b_period", rownames(fit_sum))
      ),])
...
cat("runtime: ", runtime, "\n")
cat("reps: ", reps, "\n")
cat("knots_age_cgam: ", knots_age_cgam, "\n")
cat("nknots_age_cgam: ", nknots_age_cgam, "\n")
cat("knots_age_spline: ", knots_age_spline, "\n")
cat("nknots_age_spline: ", nknots_age_spline, "\n")
...
save(knots_age_cgam, file = "knots_age_cgam.Rdata")
save(nknots_age_cgam, file = "nknots_age_cgam.Rdata")
save(knots_age_spline, file = "knots_age_spline.Rdata")
save(nknots_age_spline, file = "nknots_age_spline.Rdata")
save(knots_period, file = "knots_period.Rdata")
save(nknots_period, file = "nknots_period.Rdata")