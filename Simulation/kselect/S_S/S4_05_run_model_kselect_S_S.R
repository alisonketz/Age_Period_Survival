...
modelcode <- nimbleCode({
  ...
  tau_period ~ dgamma(0.01, 0.01)
  for (t in 1:nT_age) {
    age_effect[t] <- inprod(b_age[1:nknots_age], Z_age[t, 1:nknots_age])
  }
  for (t in 1:nT_period) {
    period_effect[t] <- inprod(b_period[1:nknots_period], Z_period[t, 1:nknots_period])
  }
...
initsFun <- function()list(tau_period = runif(1),
                          tau_age = runif(1),
...
confMCMC$removeSamplers(c("b_age", "b_period")) 
