...
initsFun <- function()list(
  period_effect = rep(0, nT_period),
  age_effect = rep(beta0_true, nT_age),
  tau_age = runif(1, 10, 11),
  tau_period = runif(1, 1, 2)
)
nimInits <- initsFun()
...
confMCMC <- configureMCMC(Rmodel,
                          ...
                          useConjugacy = FALSE,
                          enableWAIC = TRUE)
confMCMC$addSampler(target = c("tau_period", "tau_age"),
                    type = "RW_block")
